...
export function TooltipPanelContent({ elementDetails, changedKeys, highlightedEffectKey, typeMultipliers }: TooltipPanelContentProps) {
    ...
    const toTypeBadgeClass = (value?: string) => {
        if (!value || value.trim().length === 0) return "type-badge-none";
        return `type-badge-${value.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
    };

    const toTypeThemeClass = (value?: string) => {
        if (!value || value.trim().length === 0) return "tooltip-theme-none";
    ...
    const primaryBadgeClass = isSpell ? "tooltip-badge-spell" : isWeapon ? "tooltip-badge-weapon" : "";

    const baseDamage = elementDetails.damage ?? 0;
    const masteryMultiplier = typeMultipliers
        ? Math.max(
        ...
        <div className={`floating-tooltip__panel ${toTypeThemeClass(primaryThemeType)}`}>
            <div className="tooltip-container">
                ...
                <div className="element-title">
                    ...
                    <div className={`description${changedKeys?.has("description") ? " property-changed" : ""}`}>{elementDetails.description}</div>
                </div>
                <div className="tooltip-header">
                    ...
                    <div className="element-types">
                        {elementDetails.type1 ? (
                            <span className={`element-info-badge ${toTypeBadgeClass(elementDetails.type1)}${changedKeys?.has("type1") ? " property-changed" : ""}`}>{elementDetails.type1}</span>
                        ) : null}
                        {elementDetails.type2 ? (
                            <span className={`element-info-badge ${toTypeBadgeClass(elementDetails.type2)}${changedKeys?.has("type2") ? " property-changed" : ""}`}>{elementDetails.type2}</span>
                        ) : null}
                    </div>
                </div>
                    ...
                    {enhancementItems.map((item) => (
                        <span
                            ...
                            className={[
                                ...
                                changedKeys?.has(`enhancement-${item.key}`) ? "property-changed" : "",
                            ].filter(Boolean).join(" ")}
                        >
                            {item.label}
                        </span>
